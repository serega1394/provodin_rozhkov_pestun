...
public class VVector2D {
	...
	public VVector2D( double x, double y ) {
		v[0] = x;
		v[1] = y;
	}
	...
	public double angle(
		double lowerBound // in radians
	) {
		double l = length();
		if ( l <= 0 )
			return lowerBound;
		double angle = Math.asin(y()/l);
	...
	static public double dot( VVector2D a, VVector2D b ) {
	...
	static public double computeSignedAngle( VVector2D v1, VVector2D v2 ) {
		...
		double dotProduct = VVector2D.dot( v1, v2 );
        ...
        public static VVector2D reflect(VVector2D vector, VVector2D normal){
            double vDotX2 = VVector2D.dot(vector,normal)*2;
            return vector.diff(vector,vector.mult(normal, (float) vDotX2)); 
        }
}
